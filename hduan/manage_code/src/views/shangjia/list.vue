		...
		<div class="center_view">
			...
			<el-table
				...
				<el-table-column min-width="140"
					...
					label="资质类型">
					<template #default="scope">
						{{scope.row.zizhileixing}}
					</template>
				</el-table-column>
				<el-table-column label="资质证明" min-width="140" width="120" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<div v-if="scope.row.zizhizhengming">
							<el-image v-if="scope.row.zizhizhengming.substring(0,4)=='http'" preview-teleported
								:preview-src-list="[scope.row.zizhizhengming.split(',')[0]]"
								:src="scope.row.zizhizhengming.split(',')[0]" style="width:100px;height:100px"></el-image>
							<el-image v-else preview-teleported
								:preview-src-list="[$config.url+scope.row.zizhizhengming.split(',')[0]]"
								:src="$config.url+scope.row.zizhizhengming.split(',')[0]" style="width:100px;height:100px">
							</el-image>
						</div>
						<div v-else>无图片</div>
					</template>
...
<script setup>
    ...
    const chatClick = (row) => {
        ...
        if (row.shangjiazhanghao) {
            nowname.value = row.shangjiazhanghao
        }

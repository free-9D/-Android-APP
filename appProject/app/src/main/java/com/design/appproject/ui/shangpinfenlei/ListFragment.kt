...
import com.design.appproject.ui.shangpinfenlei.ListFilterDialog
...
import com.design.appproject.databinding.ShangpinfenleicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.shangpinfenlei.ListModel
import com.design.appproject.ui.shangpinfenlei.ListAdapter
 ...
 * 商品分类列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_SHANGPINFENLEI)
class ListFragment : BaseBindingFragment<ShangpinfenleicommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("商品分类","shangpinfenlei")/*查询索引*/

    override fun initEvent() {
        setBarTitle("商品分类",mHasBack)
        setBarColor("#A2B772","white")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("shangpinfenlei","新增"))|| Utils.isAuthFront("shangpinfenlei","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_SHANGPINFENLEI).navigation()
            }
            if (mIsHome){
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_SHANGPINFENLEI)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("shangpinfenlei",listOf(mListAdapter.data[position].id!!)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_SHANGPINFENLEI,map=mapOf("mId" to data[position].id!!,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun ShangpinfenleicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("shangpinfenlei","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
        ...
        mIsBack.yes {
            mListModel.page("shangpinfenlei", requestParams)
        }.otherwise {
            mListModel.list("shangpinfenlei", requestParams)
        }
    }
}
...
class CssParseHelper {
        ...
        if (cssBackground.startsWith("linear-gradient")){
            ...
            if (liList.size>=3){
                ...
                colorList.forEach {
                    val c= it.trim().split(" ")
                    if(c[0].length<=4){
                        colors.add(Color.parseColor(c[0]+c[0].takeLast(3)))
                    }else{
            ...
            val drawableList = arrayOfNulls<Triple<Int,Drawable,Triple<Int,Int,Int>>>(cblist.size)
                    ...
                    drawableList.forEachIndexed { index, pair ->
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                            if (pair!=null){
                                ...
                                layerDrawable.setLayerGravity(index, pair.first)
                                layerDrawable.setLayerSize(index,pair.third.first,pair.third.second)
                                layerDrawable.setLayerInsetLeft(index,pair.third.third)
                            }
                        }
                    }
    ...
    private fun addLayerDrawable(url:String,index:Int,drawableList:Array<Triple<Int,Drawable,Triple<Int,Int,Int>>?>,latch:CountDownLatch){
                ...
                Glide.with(com.union.union_basic.utils.AppUtils.getApp()).asDrawable().load(imgUrl)
                    .into(object : SimpleTarget<Drawable>() {
                        ...
                        override fun onResourceReady(resource: Drawable, transition: Transition<in Drawable>?) {
                            ...
                            if (imgTmp.replace(imgUrl,"").contains("/")){
                                ...
                                if (sizeList.size>=1 && sizeList[0].endsWith("%")){
                                    imgWidth = if (sizeList[0]=="100%"){
                                        mOwner.get()?.measuredWidth?:-1
                                    }else{
                                        ((mOwner.get()?.measuredWidth?:0)*(sizeList[0].substring(0,sizeList[0].length-1).toFloat()/100)).toInt()
                                    }
                                    imgHeight = (((mOwner.get()?.measuredWidth?:0)*(sizeList[0].substring(0,sizeList[0].length-1).toFloat()/100))/resource.minimumWidth *resource.minimumHeight).toInt()
                                }
                                if (sizeList.size>=2 &&sizeList[1].endsWith("%")){
                                    imgHeight = (mOwner.get()?.measuredHeight?:0)*(sizeList[1].substring(0,sizeList[1].length-1).toFloat()/100).toInt()
                            ...
                            // 设置 Drawable 的平铺效果
                            if (imgTmp.contains("repeat-x")) {
                                imgWidth = mOwner.get()?.measuredWidth?:0
                                val tiledDrawable = BitmapDrawable(mContext.resources, (resource as BitmapDrawable).bitmap)
                                tiledDrawable.setBounds(0, 0, imgWidth, imgHeight)
                                // 创建一个 LayerDrawable 来实现 repeat-x 效果
                                val layerDrawable = LayerDrawable(arrayOf(tiledDrawable))
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                                    layerDrawable.setLayerGravity(0, gravity)
                                    layerDrawable.setLayerWidth(0, imgWidth)
                                    layerDrawable.setLayerHeight(0, imgHeight)
                                }
                                drawableList[index] = Triple(gravity, layerDrawable, Triple(imgWidth, imgHeight,0))
                            } else if (imgTmp.split(" ").size==4 && imgTmp.split(" ")[2].contains("%")){
                                val vwidth = mOwner.get()?.measuredWidth?:0
                                val paddingSize = imgTmp.split(" ")[2].replace("%","").toDouble()*vwidth*0.01
                                val tiledDrawable = BitmapDrawable(mContext.resources, (resource as BitmapDrawable).bitmap)
                                tiledDrawable.setBounds(paddingSize.toInt(), 0, imgWidth, imgHeight)
                                gravity=gravity.or(Gravity.LEFT).or(Gravity.CENTER_VERTICAL)
                                drawableList[index] = Triple(gravity, tiledDrawable, Triple(imgWidth, imgHeight,paddingSize.toInt()))
                            }else {
                                drawableList[index] = Triple(gravity, resource, Triple(imgWidth, imgHeight,0))
                            }
                            latch.countDown()
                        }
                    })
        ...
        if (url.startsWith("#")){
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                drawableList[index] = Triple(Gravity.CENTER,ShapeDrawable().apply {
                    ...
                    if(url.length<=4){
                        paint.color = Color.parseColor(url+url.takeLast(3))
                    }else{
                        paint.color = Color.parseColor(url)
                    }
                },Triple(mOwner.get()?.measuredWidth?:0,mOwner.get()?.measuredHeight?:0,0))
                latch.countDown()
            }
        }
    }
    ...
    fun dispatchDraw(canvas: Canvas) {
        ...
        mOwner.get()?.let { mOwnerView ->
            ...
            mCaclHeight.isNotNullOrEmpty().yes {
                if (mCaclHeight.endsWith("_min")){
                    mOwner.get()?.minimumHeight =getCaclSize(
                        parent.measuredHeight - (parent.paddingTop + parent.paddingBottom),
                        mCaclHeight.replace("_min","")
                    )
                }else{
            ...
            mPerWidth.endsWith("%").yes {
                if (mPerWidth == "100%") {
                    ...
                    val childwidth = mOwner.get()?.toConversion<ViewGroup>()?.getChildAt(0)?.width?:0

                    if (width<childwidth){
                        width= childwidth.toFloat()
                    }
                    if (mOwnerView.tag=="avatar_tag"){
                        LoggerManager.d("width:"+width)
                        LoggerManager.d("childwidth:"+childwidth)
                    }
            ...
            mPerHeight.endsWith("%").yes {
                if (mPerHeight == "100%") {
                    ...
                    val childheight = mOwner.get()?.toConversion<ViewGroup>()?.getChildAt(0)?.height?:0
        ...
        mOwner.get()?.let {mOwnerView->
            if (shadowList.contains("inset")){//内阴影
                ...
                shadowList.find { it.startsWith("#") }?.let {
                    if(it.length<=4){
                        shadowPaint.color = Color.parseColor(it+it.takeLast(3))
                    }else{
                ...
                mOwnerView.toConversion<IQMUILayout>()?.apply {
                    ...
                    shadowList.find { it.startsWith("#") }?.let {
                        if(it.length<=4){
                            shadowColor = Color.parseColor(it+it.takeLast(3))
                        }else{
...
//                mOwnerView.marginKTX(left = realCssRight)
            ...
            if (mCssBottom != -9999f && !isSetBottom) {
                isSetBottom = true
                if (mCssBottom==-998f){
                    mOwnerView.marginKTX(top = -56.dp)
                    return
                }
...
//                mOwnerView.marginKTX(top = realCssBottom)
    ...
    private fun getPositionDistance(position: Int, newClip: Boolean = false): Int { //0上，1右，2下，3左
        ...
        var viewWidth = mOwner.get()?.measuredWidth ?: 1
        var viewHeight = mOwner.get()?.measuredHeight ?: 1
        val location = intArrayOf(0, 0)
        mOwner.get()?.getLocationOnScreen(location)
        LoggerManager.d("ag:${mOwner.get()?.tag}")
        ...
        if (parentPosition<viewWidth && position ==1){
            var parentView = 375.dp
            if (positionView?.toConversion<MyFlexBoxLayout>()?.alignItems?:0==2){
                parentView = (375/2).dp+(viewWidth/2)
            }
            parentPosition = parentView-(positionView?.toConversion<View>()?.marginLeft?:0)-(positionView?.toConversion<View>()?.marginRight?:0)
        }
    ...
    private fun getCaclSize(size: Int, cacl: String): Int {
        return if (cacl.isNotNullOrEmpty()) {
            ...
            caclList.forEachIndexed { index, s ->
                if (s.endsWith("%") || s.endsWith("vh")  || s.endsWith("vw")) {
                    caclMap[s] = (size * (s.replace("%","").replace("vh","").replace("vw","").toFloat() / 100)).toString()
                } else if (s.endsWith("rpx") || s.endsWith("px")) {
                    caclMap[s] = (s.cssdp+3.dp).toString()
                }
            }
            caclMap.forEach {

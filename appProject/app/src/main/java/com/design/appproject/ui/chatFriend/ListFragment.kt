...
import com.design.appproject.ui.chatFriend.ListFilterDialog
...
import com.design.appproject.databinding.ChatfriendcommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.chatFriend.ListModel
import com.design.appproject.ui.chatFriend.ListAdapter
 ...
 * 好友表列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_CHATFRIEND)
class ListFragment : BaseBindingFragment<ChatfriendcommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("好友表",mHasBack)
        setBarColor("#A2B772","white")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("chat_friend","新增"))|| Utils.isAuthFront("chat_friend","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_CHATFRIEND).navigation()
            }
            if (mIsHome){
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_CHATFRIEND)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("chatfriend",listOf(mListAdapter.data[position].id!!)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_CHATFRIEND,map=mapOf("mId" to data[position].id!!,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun ChatfriendcommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
            mListModel.page("chatfriend", requestParams)
        }.otherwise {
            mListModel.list("chatfriend", requestParams)
        }
    }
}
...
object HomeRepository : BaseRepository() {
    ...
    fun defaultAddress(userId: Long) = fireX {
        homeService.defaultAddress(userId).await()
    }
    inline fun <reified T> query(tableName: String, map: Map<String, String>) = fireY<T> {
        homeService.query(tableName = tableName, map).await()
    }

    inline fun <reified T> page2(tableName: String, map: Map<String, String>) = fireList<T> {
        homeService.page2(tableName = tableName, map).await()
    }

    inline fun <reified T> mlist(map: Map<String, String>) = fireList<T> {
        homeService.mlist(map).await()
    }
}
...
class AddOrUpdateActivity:BaseBindingActivity<YonghuaddorupdateLayoutBinding>() {
    ...
    private fun YonghuaddorupdateLayoutBinding.submit() {
        ...
        moneyEt.inputType = InputType.TYPE_CLASS_NUMBER
        mYonghuItemBean.money = moneyEt.text.toString().toDoubleOrNull()?:0.0
        ...
        if(mYonghuItemBean.yonghuxingming.isNullOrEmpty()){
            "用户姓名不能为空".showToast()
            return
        }
        if(mYonghuItemBean.touxiang.isNullOrEmpty()){
            "头像不能为空".showToast()
            return
        }
    ...
    private fun YonghuaddorupdateLayoutBinding.setData(){
        ...
        if (mYonghuItemBean.yonghuxingming.isNotNullOrEmpty()){
            yonghuxingmingEt.setText(mYonghuItemBean.yonghuxingming.toString())
        }
        ...
        if (mYonghuItemBean.money>=0){
            moneyEt.setText(mYonghuItemBean.money.toString())
        }
    }
}
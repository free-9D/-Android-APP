...
import com.design.appproject.ui.fankuiyijian.ListFilterDialog
...
import com.design.appproject.databinding.FankuiyijiancommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.fankuiyijian.ListModel
import com.design.appproject.ui.fankuiyijian.ListAdapter
 ...
 * 反馈意见列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_FANKUIYIJIAN)
class ListFragment : BaseBindingFragment<FankuiyijiancommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("反馈标题","fankuibiaoti")/*查询索引*/

    override fun initEvent() {
        setBarTitle("反馈意见",mHasBack)
        setBarColor("#A2B772","white")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("fankuiyijian","新增"))|| Utils.isAuthFront("fankuiyijian","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_FANKUIYIJIAN).navigation()
            }
            if (mIsHome){
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_FANKUIYIJIAN)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("fankuiyijian",listOf(mListAdapter.data[position].id!!)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_FANKUIYIJIAN,map=mapOf("mId" to data[position].id!!,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun FankuiyijiancommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("fankuibiaoti","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
            params.remove("fankuibiaoti")
        }
        val requestParams = mutableMapOf<String,String>()
        requestParams.putAll(params)
        searchParams?.let { requestParams.putAll(it) }
        mIsBack.yes {
            mListModel.page("fankuiyijian", requestParams)
        }.otherwise {
            mListModel.list("fankuiyijian", requestParams)
        }
    }
}
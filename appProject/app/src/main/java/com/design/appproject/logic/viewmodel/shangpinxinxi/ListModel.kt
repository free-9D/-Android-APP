package com.design.appproject.logic.viewmodel.shangpinxinxi
...
class ListModel : ViewModel() {
    ...
    val pageLiveData = Transformations.switchMap(pageData) { request ->
        pageData.value?.let {
            HomeRepository.page<ShangpinxinxiItemBean>(it[0] as String, it[1] as Map<String, String>)
        }
    }
    ...
    val listLiveData = Transformations.switchMap(listData) { request ->
        listData.value?.let {
            HomeRepository.list<ShangpinxinxiItemBean>(it[0] as String, it[1] as Map<String, String>)
        }
    }

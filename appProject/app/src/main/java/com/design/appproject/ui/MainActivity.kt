...
class MainActivity : BaseBindingActivity<ActivityMainLayoutBinding>() {
    ...
    private val fragments = mutableListOf(
        ...
        ARouter.getInstance().build(CommonArouteApi.PATH_FRAGMENT_LIST_SHANGPINXINXI).withBoolean("mHasBack",false).withBoolean("mIsHome",true).navigation() as Fragment,
        ARouter.getInstance().build(CommonArouteApi.PATH_FRAGMENT_LIST_DUANSHIPIN).withBoolean("mHasBack",false).withBoolean("mIsHome",true).navigation() as Fragment,
    ...
    public fun reload(){//重新加载界面
        fragments.forEach {
            it.toConversion<BaseBindingFragment<ViewBinding>>()?.initData()
        }
    }

...
import com.design.appproject.ui.duanshipin.ListFilterDialog
...
import com.design.appproject.databinding.DuanshipincommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.duanshipin.ListModel
import com.design.appproject.ui.duanshipin.ListAdapter
 ...
 * 短视频列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_DUANSHIPIN)
class ListFragment : BaseBindingFragment<DuanshipincommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("标题","biaoti")/*查询索引*/

    override fun initEvent() {
        setBarTitle("短视频",mHasBack)
        ...
        binding.apply {
            ...
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("duanshipin","新增"))|| Utils.isAuthFront("duanshipin","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_DUANSHIPIN).navigation()
            }
            if (mIsHome){
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_DUANSHIPIN)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("duanshipin",listOf(mListAdapter.data[position].id!!)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_DUANSHIPIN,map=mapOf("mId" to data[position].id!!,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun DuanshipincommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("biaoti","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
            params.remove("biaoti")
        }
        val requestParams = mutableMapOf<String,String>()
        requestParams.putAll(params)
        searchParams?.let { requestParams.putAll(it) }
        mIsBack.yes {
            mListModel.page("duanshipin", requestParams)
        }.otherwise {
            mListModel.list("duanshipin", requestParams)
        }
    }
}
...
class ForgetActivity : BaseBindingActivity<ActivityForgetLayoutBinding>() {
    ...
    var yonghuItemBean = YonghuItemBean()
    var tableName=""
    var panswer=""/**密保答案*/
                ...
                verify().yes {
                    when (mStep) {
                        ...
                        3 -> {
                            ...
                            when(tableName){
                                "yonghu"-> {
                                    yonghuItemBean.mima = passwordEt.text.toString()
                                    mForgetViewModel.update("yonghu",yonghuItemBean)
                                }
                            }
                        }
                        else -> {
                            showLoading()
                            when(tableName){
                                "yonghu"->mForgetViewModel.yonghuSecurity(tableName,usernameEt.text.toString())
                            }
                        }
                    }
                }
    ...
    override fun initData() {
        super.initData()
        mForgetViewModel.yonghuSecurityLiveData.observeKt {
            it.getOrNull()?.let {
                if (it.data==null){
                    "用户不存在".showToast()
                }else {
                    yonghuItemBean = it.data
                    binding.pquestionTv.setText(it.data.pquestion)
                    panswer = it.data.panswer
                    mStep = 2
                    binding.updateView()
                }
            }

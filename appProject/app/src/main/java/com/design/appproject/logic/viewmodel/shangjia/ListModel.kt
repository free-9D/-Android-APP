package com.design.appproject.logic.viewmodel.shangjia
...
class ListModel : ViewModel() {
    ...
    val pageLiveData = Transformations.switchMap(pageData) { request ->
        pageData.value?.let {
            HomeRepository.page<ShangjiaItemBean>(it[0] as String, it[1] as Map<String, String>)
        }
    }
    ...
    val listLiveData = Transformations.switchMap(listData) { request ->
        listData.value?.let {
            HomeRepository.list<ShangjiaItemBean>(it[0] as String, it[1] as Map<String, String>)
        }
    }

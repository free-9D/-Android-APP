...
import com.design.appproject.ui.yijianfankui.ListFilterDialog
...
import com.design.appproject.databinding.YijianfankuicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.yijianfankui.ListModel
import com.design.appproject.ui.yijianfankui.ListAdapter
 ...
 * 意见反馈列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_YIJIANFANKUI)
class ListFragment : BaseBindingFragment<YijianfankuicommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("意见反馈",mHasBack)
        setBarColor("#A2B772","white")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("yijianfankui","新增"))|| Utils.isAuthFront("yijianfankui","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_YIJIANFANKUI).navigation()
            }
            if (mIsHome){
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_YIJIANFANKUI)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("yijianfankui",listOf(mListAdapter.data[position].id!!)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_YIJIANFANKUI,map=mapOf("mId" to data[position].id!!,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun YijianfankuicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
            mListModel.page("yijianfankui", requestParams)
        }.otherwise {
            mListModel.list("yijianfankui", requestParams)
        }
    }
}